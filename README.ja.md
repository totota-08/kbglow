...
**GUIアプリ**(Claudeデスクトップ、ChatGPTアプリ)にはフック機構がないため、`kbglow-setup --watch` が常駐ウォッチャーを設置します。アプリが通知を出したら点滅し、フォーカスするか通知を消すと消灯。唯一の手動ステップ(フルディスクアクセスの付与)もガイドしてくれて、効いた瞬間に確認表示が出ます。注意点は2つ: 点滅開始は通知の約5〜10秒後(macOSの遅延書き込みのため)、kbglow更新後は許可の再付与が必要(バイナリの署名に紐付くため。更新時に案内が出ます)。再付与を不要にする方法は次の段落へ。

**更新をまたいでフルディスクアクセスを維持する(任意)。** npm のビルドは ad-hoc 署名のため、更新のたびに macOS からは別のバイナリに見えて許可が外れます。キーチェーンに `kbglow-local` という名前の自己署名コード署名証明書があれば、`kbglow-setup --watch` 実行時と、(ウォッチャー設置中の)npm 更新時に自動でその証明書で署名し直すので、署名が安定して許可が維持されます。作成は一度だけ: **キーチェーンアクセス → 証明書アシスタント → 証明書を作成…** → 名前: `kbglow-local`、固有名のタイプ: *自己署名ルート*、証明書のタイプ: *コード署名*。初回の署名時にキーチェーンのダイアログが出たら「常に許可」を選択。そのあと `kbglow-setup --watch` を再実行して、最後に一度だけフルディスクアクセスを付与してください。
